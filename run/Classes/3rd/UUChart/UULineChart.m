...
- (void)addPoint:(CGPoint)point index:(NSInteger)index isShow:(BOOL)isHollow value:(CGFloat)value
{
    ...
    CustomButton *button = [[CustomButton alloc] initWithFrame:view.bounds Title:nil WhenTouchUpinside:^(id sender) {
        NSLog(@"%f",value);
        
        UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 40, 40)];
        UILabel *ylabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 20, 20)];
        UILabel *xlabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 20, 2, 20)];
        
        ylabel.text = [NSString stringWithFormat:@"%.2f",value];
        xlabel.text = [NSString stringWithFormat:@"%ld公里",index];
        [view addSubview:xlabel];
        [view addSubview:ylabel];
        
    }];
    button.layer.cornerRadius = 4;
    button.layer.borderColor = [[_colors objectAtIndex:index] CGColor]?[[_colors objectAtIndex:index] CGColor]:UUGreen.CGColor;
